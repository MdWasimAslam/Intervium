# Prompt — generate seed questions for `db/load-questions.ts`

Paste the block below into Claude (claude.ai or API). Edit the **SCOPE** section
each run to control how much it generates, save the JSON it returns to
`db/questions.json`, then load it:

```
npm run db:load-questions -- --file db/questions.json --dry-run   # validate
npm run db:load-questions -- --file db/questions.json             # write
```

The loader is idempotent and dedupes, so you can run the prompt many times
(different scopes), append/replace the file, and re-load safely.
