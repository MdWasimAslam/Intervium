 ...
 *
 * Middleware does NOT run on /api/*, so this self-guards: admin-only. Returns
 * clean status codes (not redirects) so the client fetch can react. Always Node
 * runtime (DB + fs access) and never cached.
 */
